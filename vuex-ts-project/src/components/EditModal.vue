    ...
    <div class="modal-content">
      <form @submit.prevent="updateTask">
        ...
        <div class="field">
          ...
          <div class="control">
            <input v-model="title" class="input" type="text" placeholder="Enter task" />
          </div>
        </div>
...
export default {
  ...
  setup(props: any) {
    ...
    const updateTask = () => {
      if (state.title === '' || state.description === '' || state.createdBy === '' || state.assignedTo === '')
        return

      const task: TaskItem = { id: props.id, title: state.title, description: state.description, createdBy: state.createdBy, assignedTo: state.assignedTo, completed: false, editing: false }
